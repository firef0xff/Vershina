...
__fastcall OPCRW::OPCRW(void) // конструктор
{
   ...
   r1 = pIOPCServer->AddGroup(pGr1Name, true, 500, 1, &TimeBias,
      &PercentDeadband, LOCALE_ID, &GrpSrvHandle, &RevisedUpdateRate,
      IID_IOPCItemMgt, (LPUNKNOWN*)&pIOPCItemMgt1);
   if (r1 == S_OK)
   {
   ...
   else
   {
      ...
      LogPrint("Ошибка при добавлении переменных группы 1: " +
   ...
   if (r1 >= 0)
   {
      LogPrint("Получен указатель на IOPCSyncIO1 для переменных группы \"" +
         String(pGr1Name) + "\"", clWhite);
   }
   else
   {
   ...
   r1 = pIOPCServer->AddGroup(pGr3Name, true, 500, 3, &TimeBias,
      &PercentDeadband, LOCALE_ID, &GrpSrvHandle, &RevisedUpdateRate,
      IID_IOPCItemMgt, (LPUNKNOWN*)&pIOPCItemMgt3);
   if (r1 == S_OK)
   {
   ...
   {
      pIOPCServer->GetErrorString(pErrors[0], LOCALE_ID, &ErrorStr);
      LogPrint("Ошибка при добавлении переменных группы 3: " +
         String(ErrorStr), clRed);
      CoTaskMemFree(ErrorStr);
   ...
   if (r1 >= 0)
   {
      LogPrint("Получен указатель на IOPCSyncIO3 для переменных группы \"" +
         String(pGr3Name) + "\"", clWhite);
   }
   else
   {
   ...
   r1 = pIOPCServer->AddGroup(pGr5Name, true, 500, 5, &TimeBias,
      &PercentDeadband, LOCALE_ID, &GrpSrvHandle, &RevisedUpdateRate,
      IID_IOPCItemMgt, (LPUNKNOWN*)&pIOPCItemMgt5);
   if (r1 == S_OK)
   {
   ...
   {
      if (i < GR51ITEMSNUM)
         wsStr = AddrDB7 + String(i * 4 + 2);
      else
         wsStr = AddrDB21 + String((i - GR51ITEMSNUM) * 4 + 2);
   ...
   {
      pIOPCServer->GetErrorString(pErrors[0], LOCALE_ID, &ErrorStr);
      LogPrint("Ошибка при добавлении переменных группы 5: " +
         String(ErrorStr), clRed);
      CoTaskMemFree(ErrorStr);
   ...
   if (r1 >= 0)
   {
      LogPrint("Получен указатель на IOPCSyncIO5 для переменных группы \"" +
         String(pGr5Name) + "\"", clWhite);
   }
   else
   {
   ...
   r1 = pIOPCServer->AddGroup(pGr6Name, true, 500, 6, &TimeBias,
      &PercentDeadband, LOCALE_ID, &GrpSrvHandle, &RevisedUpdateRate,
      IID_IOPCItemMgt, (LPUNKNOWN*)&pIOPCItemMgt6);
   if (r1 == S_OK)
   {
   ...
   {
      pIOPCServer->GetErrorString(pErrors[0], LOCALE_ID, &ErrorStr);
      LogPrint("Ошибка при добавлении переменных группы 6: " +
         String(ErrorStr), clRed);
      CoTaskMemFree(ErrorStr);
   ...
   if (r1 >= 0)
   {
      LogPrint("Получен указатель на IOPCSyncIO6 для переменных группы \"" +
         String(pGr6Name) + "\"", clWhite);
   }
   else
   {
   ...
   float *pfw) // переменная типа float из группы 3
{
   ...
   {
      LogPrint(
         "Write Gr1 bool 1 ffm BenchControl: can not find item to write! (iter<0)",
         clRed);
      return 0;
   }
   if (iter2 < 0)
   {
      LogPrint(
         "Write Gr1 bool 2 fm BenchControl: can not find item to write! (iter<0)",
         clRed);
      return 0;
   }
   phServer.resize(2);
   ...
   {
      pIOPCServer->GetErrorString(pWErrors[0], LOCALE_ID, &ErrorStr);
      LogPrint("Write Gr1 fm BenchControl: bool item1 writing error: " +
         String(ErrorStr), clRed);
      pIOPCServer->GetErrorString(pWErrors[1], LOCALE_ID, &ErrorStr);
      LogPrint("Write Gr1 fm BenchControl: bool item2 writing error: " +
         String(ErrorStr), clRed);
   ...
   int *piw4) // переменная типа int из группы 3
{
   ...
   {
      LogPrint(
         "Write Gr3 float from TestParamWrite: can not find item1 to write! (iter<0)",
         clRed);
      return 0;
   }
   std::vector<OPCHANDLE>phServer;
   ...
   {
      LogPrint(
         "Write Gr3 int from TestParamWrite: can not find item2 to write! (iter<0)",
         clRed);
      return 0;
   }
   ...
   {
      LogPrint(
         "Write Gr3 int from TestParamWrite: can not find item3 to write! (iter<0)",
         clRed);
      return 0;
   }
   ...
   {
      LogPrint(
         "Write Gr3 int from TestParamWrite: can not find item4 to write! (iter<0)",
         clRed);
      return 0;
   }
   ...
   {
      pIOPCServer->GetErrorString(pWErrors[0], LOCALE_ID, &ErrorStr);
      LogPrint("Write Gr3 int from TestParamWrite: item1 writing error: " +
         String(ErrorStr), clRed);
      pIOPCServer->GetErrorString(pWErrors[1], LOCALE_ID, &ErrorStr);
      LogPrint("Write Gr3 int from TestParamWrite: item2 writing error: " +
         String(ErrorStr), clRed);
      pIOPCServer->GetErrorString(pWErrors[2], LOCALE_ID, &ErrorStr);
      LogPrint("Write Gr3 int from TestParamWrite: item3 writing error: " +
         String(ErrorStr), clRed);
      pIOPCServer->GetErrorString(pWErrors[3], LOCALE_ID, &ErrorStr);
      LogPrint("Write Gr3 int from TestParamWrite: item4 writing error: " +
         String(ErrorStr), clRed);
      pIOPCServer->GetErrorString(pWErrors[4], LOCALE_ID, &ErrorStr);
      LogPrint("Write Gr3 int from TestParamWrite: item5 writing error: " +
         String(ErrorStr), clRed);
...
int __fastcall OPCRW::ReadGr1(void) // чтение переменных группы 1
{
   ...
   for (int i = 0; i < GR1ITEMSNUM; i++)
   {
      phServer[i] = pItemResult1[i].hServer;
   }
   r1 = pIOPCSyncIO1->Read(OPC_DS_CACHE, GR1ITEMSNUM, &(*phServer.begin()),
      &pItemValue, &pRErrors);
   if (r1 == S_OK)
   {
      for (int i = 0; i < GR1BOOLITEMSNUM; i++)
      ...
      for (int i = 0; i < GR1FLTITEMSNUM; i++)
         fGr1[i] = pItemValue[i + GR1BOOLITEMSNUM + GR1INTITEMSNUM]
            .vDataValue.fltVal;
...
int __fastcall OPCRW::ReadGr3(void) // чтение переменных группы 3
{
   ...
   for (int i = 0; i < GR3ITEMSNUM; i++)
   {
      phServer[i] = pItemResult3[i].hServer;
   }
   r1 = pIOPCSyncIO3->Read(OPC_DS_CACHE, GR3ITEMSNUM, &(*phServer.begin()),
      &pItemValue, &pRErrors);
   if (r1 == S_OK)
   {
      ...
      CoTaskMemFree(pItemValue);
      LogPrint("Read Group 3 i0=" + String(iDB10[0]) + ", i1=" +
         String(iDB10[1]), clLime);
...
int __fastcall OPCRW::ReadGr5(void) // чтение переменных группы 5
{
   ...
   r1 = pIOPCSyncIO5->Read(OPC_DS_CACHE, GR5ITEMSNUM, &(*phServer.begin()),
      &pItemValue, &pRErrors);
   ...
   if (r1 == S_OK)
   {
      ...
      {
         if (i < GR51ITEMSNUM)
            poll_step_TA[i] = pItemValue[i].vDataValue.lVal;
         else
            step_TA[i - GR51ITEMSNUM] = pItemValue[i].vDataValue.lVal;
      }
      for (int i = 0; i < GR5ITEMSNUM; i++)
      {
         VariantClear(&pItemValue[i].vDataValue);
      ...
      CoTaskMemFree(pItemValue);
      LogPrint("poll_step_TA[0]=" + String(poll_step_TA[0]), clLime);
      // debug print
...
int __fastcall OPCRW::ReadGr6(void) // чтение переменных группы 6
{
   ...
   r1 = pIOPCSyncIO6->Read(OPC_DS_CACHE, GR6ITEMSNUM, &(*phServer.begin()),
      &pItemValue, &pRErrors);
   ...
   if (r1 == S_OK)
   {
      ...
      {
         if (i < MAXNUMOFSTEPS)
            setting_A[0][i] = pItemValue[i].vDataValue.fltVal;
         else
            setting_A[1][i - MAXNUMOFSTEPS] = pItemValue[i].vDataValue.fltVal;
      }
      for (int i = 0; i < GR6ITEMSNUM; i++)
      {
         VariantClear(&pItemValue[i].vDataValue);
      ...
      CoTaskMemFree(pItemValue);
      LogPrint("setting_A[0][0]=" + FloatToStrF(setting_A[0][0], ffFixed, 6, 2),
         clLime); // debug print
      LogPrint("setting_A[1][0]=" + FloatToStrF(setting_A[1][0], ffFixed, 6, 2),
         clLime); // debug print
...
int __fastcall OPCRW::ReadGr9(void) // чтение переменных группы 9
{
   ...
   r1 = pIOPCSyncIO9->Read(OPC_DS_CACHE, GR5ITEMSNUM, &(*phServer.begin()),
      &pItemValue, &pRErrors);
...
int __fastcall OPCRW::WriteGr1(float *piw) // запись float переменной группы 1
{
   ...
   if (iter < 0)
   {
      ...
      iter += GR1BOOLITEMSNUM + GR1INTITEMSNUM;
   std::vector<OPCHANDLE>phServer;
   phServer.resize(1);
   phServer[0] = pItemResult1[iter].hServer;
   values[0].vt = VT_R4;
...
int __fastcall OPCRW::WriteGr3(float *pfw)
   // запись вещественной переменной группы 3
{
#ifdef _NO_Write
   return 1;
#endif
   ...
   {
      pIOPCServer->GetErrorString(pWErrors[0], LOCALE_ID, &ErrorStr);
      LogPrint("Write Gr3 float: item writing error: " +
         String(ErrorStr), clRed);
...
int __fastcall OPCRW::WriteGr5(void) // запись целых массивов группы 5
{
   ...
   for (int i = 0; i < GR5ITEMSNUM; i++)
   {
      ...
      values[i].vt = VT_I4;
      if (i < GR51ITEMSNUM)
         values[i].lVal = poll_step_TA[i];
      else
         values[i].lVal = step_TA[i - GR51ITEMSNUM];
   }
   r1 = pIOPCSyncIO5->Write(GR5ITEMSNUM, &(*phServer.begin()), values,
      &pWErrors);
...
int __fastcall OPCRW::WriteGr6(void) // запись вещественного массива группы 6
{
   ...
   for (int i = 0; i < GR6ITEMSNUM; i++)
   {
      ...
      values[i].vt = VT_R4;
      if (i < MAXNUMOFSTEPS)
         values[i].fltVal = setting_A[1][i];
      else
         values[i].fltVal = setting_A[0][i - MAXNUMOFSTEPS];
   }
   r1 = pIOPCSyncIO6->Write(GR6ITEMSNUM, &(*phServer.begin()), values,
      &pWErrors);
