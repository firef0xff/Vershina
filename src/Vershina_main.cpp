...
void TmfRB::ShowCommonParam(void) // отображение общих параметров
{
   ...
   if (mPosB.CurrSMode == 2)
   {
      ...
      if (mPosB.cur_step > 1)
      {
         if (mPosB.mTyre.TestMode == 0)
         {
            ...
            stP2CParL7C3->Caption = AnsiString(dt::mSecToHMSStr(gr2p2.next_step_time).c_str());
   ...
   if (pcRB->ActivePage == tsCurrentStatus)
   {
      ...
      stP1CParL1C5->Caption = AnsiString(dt::mSecToHMSStr(gr2p2.fakt_time).c_str());
      ...
      stP1CParL8C4->Caption = String(gr2p2.step_change);
      stP1CParL8C3->Caption = String(gr2p1.next_step_change);
      stP1CParL8C6->Caption = String(gr2p2.next_step_change);
   }
   ...
   if (pcRB->ActivePage == tsManual)
   {
      ...
      if (!leSetLoad2->Focused())
      {
         ...
         leSetLoad2->Text = FloatToStrF(gr3p2.Loading, ffFixed, 5, 2);
      }
   }
   // аварийные установки
   if (pcRB->ActivePage == tsEmSettings)
   {
      ...
      leEmMaxLoad_2->Text = FloatToStrF(gr3p2.max_load, ffFixed, 5, 1);
      ...
      leEmMinLoad_2->Text = FloatToStrF(gr3p2.min_load, ffFixed, 5, 1);

      leEmMinTemp_1->Text = FloatToStrF(gr3p1.min_temp, ffFixed, 5, 1);
      leEmMinTemp_2->Text = FloatToStrF(gr3p2.min_temp, ffFixed, 5, 1);

      leEmMaxTemp_1->Text = FloatToStrF(gr3p1.max_temp, ffFixed, 5, 1);
      leEmMaxTemp_2->Text = FloatToStrF(gr3p2.max_temp, ffFixed, 5, 1);
   }
}
...
void __fastcall TmfRB::OPCControlStartExec(void)
{
   ...
   if ( !ShowTimer->Enabled )
   {
      ...
      mPosB.old_step = gr2p2.step_write;
   ...
   // расположение компонент на панели ручного управления
{
   ...
   lSpeed0->Width = 30;
   lSpeed0->Height = 20;
   lSpeed25->Left = TickLeft;
   lSpeed25->Top = tbDSTop - TickdH3;
   lSpeed25->Width = 30;
   lSpeed25->Height = 20;
   lSpeed50->Left = TickLeft;
   lSpeed50->Top = tbDSTop - TickdH3 * 2;
   lSpeed50->Width = 30;
   lSpeed50->Height = 20;
   lSpeed75->Left = TickLeft;
   lSpeed75->Top = tbDSTop - TickdH3 * 3;
   lSpeed75->Width = 30;
   lSpeed75->Height = 20;
   lSpeed100->Left = TickLeft;
   lSpeed100->Top = tbDSTop - TickdH3 * 4;
   lSpeed100->Width = 30;
   lSpeed100->Height = 20;
   lSpeed125->Left = TickLeft;
   lSpeed125->Top = tbDSTop - TickdH3 * 5;
   lSpeed125->Width = 30;
   lSpeed125->Height = 20;
   lSpeed150->Left = TickLeft;
   lSpeed150->Top = tbDSTop - TickdH3 * 6;
   lSpeed150->Width = 30;
   lSpeed150->Height = 20;
   lSpeed175->Left = TickLeft;
   lSpeed175->Top = tbDSTop - TickdH3 * 7;
   lSpeed175->Width = 30;
   lSpeed175->Height = 20;
   lSpeed200->Left = TickLeft;
   lSpeed200->Top = tbDSTop - TickdH3 * 8;
   lSpeed200->Width = 30;
   lSpeed200->Height = 20;
   lSpeed225->Left = TickLeft;
   lSpeed225->Top = tbDSTop - TickdH3 * 9;
   lSpeed225->Width = 30;
   lSpeed225->Height = 20;
   lSpeed250->Left = TickLeft;
   lSpeed250->Top = tbDSTop - TickdH3 * 10;
   lSpeed250->Width = 30;
   lSpeed250->Height = 20;
   lSpeed275->Left = TickLeft;
   lSpeed275->Top = tbDSTop - TickdH3 * 11;
   lSpeed275->Width = 30;
   lSpeed275->Height = 20;
   lSpeed300->Left = TickLeft;
   ...
   // отобразить шапку протокола в панели поз. А
{
   ...
   leManDateA->Text = AnsiString( dt::ToString(mPosA.mTyre.ManufactDate, dt::DEFAULT_DATE_FORMAT).c_str() ); // DateToStr(mPosA.mTyre.ManufactDate);
   ...
   leFormNoA->Text = AnsiString(mPosA.mTyre.FormNo.c_str());
   ...
   // отобразить шапку протокола в панели поз. Б
{
   ...
   leManDateB->Text = AnsiString(dt::ToString(mPosB.mTyre.ManufactDate, dt::DEFAULT_DATE_FORMAT).c_str()); // DateToStr(mPosB.mTyre.ManufactDate);
   ...
   leFormNoB->Text = AnsiString(mPosB.mTyre.FormNo.c_str());
   ...
   // прочитать данные протокола из экрана в InpTyre
{
   ...
   InpTyre.ManufactDate = dt::FromString( AnsiString(meManDate->EditText).c_str(), dt::DEFAULT_DATE_FORMAT );
   InpTyre.SerialNo = StrToI(leSeralNo->Text);
   InpTyre.PerfSpecNo = StrToI(lePerfSpecNo->Text);
   InpTyre.OrderNo = StrToI(leOrderNo->Text);
   InpTyre.FormNo = AnsiString(leFormNo->Text).c_str();
   ...
   // отобразить данные протокола из InpTyre на экране
{
   ...
   meManDate->EditText = AnsiString(dt::ToString(InpTyre.ManufactDate, dt::DEFAULT_DATE_FORMAT).c_str());
   leSeralNo->Text = String(InpTyre.SerialNo);
   lePerfSpecNo->Text = String(InpTyre.PerfSpecNo);
   leOrderNo->Text = String(InpTyre.OrderNo);
   leFormNo->Text = AnsiString(InpTyre.FormNo.c_str());
...
void __fastcall TmfRB::btEmSettingsClick(TObject *Sender)
{
   ...
   try
   {
      ...
      value += CheckLoad( leEmMaxLoad_2R, gr3p2.max_load);
      ...
      value += CheckLoad( leEmMinLoad_2R, gr3p2.min_load);

      gr3p1.min_temp = leEmMinTemp_1R->Text.Trim().ToDouble();
      gr3p1.max_temp = leEmMaxTemp_1R->Text.Trim().ToDouble();
      gr3p2.min_temp = leEmMinTemp_2R->Text.Trim().ToDouble();
      gr3p2.max_temp = leEmMaxTemp_2R->Text.Trim().ToDouble();
      gr3p1.Write();
      gr3p2.Write();
      cmnp.Write();
