...
Tyre __fastcall Tyre:: operator = (Tyre op2) // переопределение присваивания
{
   ...
   Size = op2.Size;
   Model = op2.Model;
   TestProcedure = op2.TestProcedure;
   StandName = op2.StandName;
   Manufacturer = op2.Manufacturer;
   DrumDiameter = op2.DrumDiameter;
   TestCustomer = op2.TestCustomer;
   ManufactDate = op2.ManufactDate;
   FormNo = op2.FormNo;
   OrderNo = op2.OrderNo;
   PerfSpecNo = op2.PerfSpecNo;
   SerialNo = op2.SerialNo;
   LoadIndex = op2.LoadIndex;
   MaxLoad = op2.MaxLoad;
   SpeedInd = op2.SpeedInd;
   MaxSpeed = op2.MaxSpeed;
   StaticR = op2.StaticR;
   OuterD = op2.OuterD;
   WheelRim = op2.WheelRim;
   MaxLoadPress = op2.MaxLoadPress;
   ProfileWide = op2.ProfileWide;
   Type = op2.Type;
   CurrentLoad = op2.CurrentLoad;
   InitPressure = op2.InitPressure;
   CurrentSpeed = op2.CurrentSpeed;
   TestMode = op2.TestMode;
   TotalTime = op2.TotalTime;
   TotalS = op2.TotalS;
   StepsNo = op2.StepsNo;
   PollsNo = op2.PollsNo;
   for (int i = 0; i < 250; i++)
...
void __fastcall Tyre::WriteTyreToFile(String fname)
...
void __fastcall Tyre::ReadTyreFmFile(String fname)
...
void __fastcall Tyre::PrintProtToFile(String fname, String side)
   // печать протокола испытаний в файл fname
{
   ...
   fprintf(fprint,
      "          ПРОТОКОЛ ИСПЫТАНИЙ № ______" /* %d */ " НА СТЕНДЕ %s\n"
      /* ,ProtNo */ , AnsiString(StandName + side).c_str());
   fprintf(fprint, "     Изготовитель: %10s     Заказчик: %s  Заказ №: %5d\n",
      AnsiString(Manufacturer).c_str(), AnsiString(TestCustomer).c_str(),
      OrderNo);
   ...
   if (Type == 0)
      fprintf(fprint,
      "               ХАРАКТЕРИСТИКИ ИСПЫТЫВАЕМОЙ ШИНЫ\n" /* РАДИАЛЬНОЙ ШИНЫ\n" */
         );
   else
      fprintf(fprint,
      "               ХАРАКТЕРИСТИКИ ИСПЫТЫВАЕМОЙ ШИНЫ\n" /* ДИАГОНАЛЬНОЙ ШИНЫ\n" */
   ...
   fprintf(fprint,
      "   Индекс нагрузки: %10s    Максимальная нагрузка: %6.2f кН \n",
      AnsiString(LoadIndex).c_str(), MaxLoad);
   fprintf(fprint,
      "Категория скорости: %10s    Максимальная скорость: %6.2f км/час \n",
      AnsiString(SpeedInd).c_str(), MaxSpeed);
   fprintf(fprint,
      " Давление при Qmax: %4.2f кПа        Наружный диаметр: %6.2f мм \n",
      MaxLoadPress, OuterD);
   fprintf(fprint,
      "Статический радиус: %6.2f мм            Ширина профиля: %6d мм \n",
      StaticR, ProfileWide);
   fprintf(fprint,
      "   Давление в шине: %6.1f кПа                   Обод: %10s\n\n",
   ...
   fprintf(fprint,
      "+-----+---------+--------+--------+--------+---------+-----------+\n");
   fprintf(fprint,
      "|Опрос|  время  |скорость|путь, км|нагрузка| радиус  |Температура|\n");
   fprintf(fprint,
      "|     |  чч:мм  | км/час |        |  кН    |   мм    |    С      |\n");
   fprintf(fprint,
      "+-----+---------+--------+--------+--------+---------+-----------+\n");
   for (int i = 0; i < PollsNo; i++)
   {
      ...
      { // это не даст печататься пустым строкам данных (пожелание заказчика)
         fprintf(fprint,
            "| %03d |%7s  | %6.2f | %6.0f | %6.2f | %5.2f |    %5.1f  |\n",
            i + 1, AnsiString(mSecToHMSStr(rT[i])).c_str(), rV[i], rS[i], rL[i],
            rR[i], rTemp[i]);
      }
   }
   fprintf(fprint,
...
void __fastcall Tyre::PrintProtocol(TPrinter *pprt, String side)
   // печать протокола на принтер pprt
{
   ...
   wstr[LineCnt] =
      "ПРОТОКОЛ ИСПЫТАНИЙ № ______" /* String(ProtNo)+ */ " НА СТЕНДЕ " +
      StandName + side;
   left[LineCnt] = abs(prtWidth - pprt->Canvas->TextWidth(wstr[LineCnt])) / 2 +
      LeftMarg;
   wstr[++LineCnt] = "     Изготовитель: " + Manufacturer + "     Заказчик: " +
      TestCustomer + "  Заказ №: " + String(OrderNo);
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "           Модель: " + Model +
      "            Размер: " + Size;
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "Дата изготовления: " + CustomDate() +
      "     Порядковый №: " + String(SerialNo);
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "     Тех. задание: " + String(PerfSpecNo) +
      "   Методика: " + TestProcedure;
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = " Диаметр барабана: " + String(DrumDiameter) +
      "мм       Форма №: " + String(FormNo);
   left[LineCnt] = LeftMarg;
   if (Type == 0)
      wstr[++LineCnt] =
         "ХАРАКТЕРИСТИКИ ИСПЫТАННОЙ ШИНЫ" /* РАДИАЛЬНОЙ ШИНЫ" */ ;
   else
      wstr[++LineCnt] =
         "ХАРАКТЕРИСТИКИ ИСПЫТАННОЙ ШИНЫ" /* ДИАГОНАЛЬНОЙ ШИНЫ" */ ;
   left[LineCnt] = abs(prtWidth - pprt->Canvas->TextWidth(wstr[LineCnt])) / 2 +
      LeftMarg;
   wstr[++LineCnt] = "   Индекс нагрузки: " + LoadIndex +
      "   Максимальная нагрузка: " + FloatToStrF(MaxLoad, ffFixed, 6, 2) + "кН";
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "Категория скорости: " + SpeedInd +
      "   Максимальная скорость: " + FloatToStrF(MaxSpeed, ffFixed, 6, 2) +
      "км/час";
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = " Давление при Qmax: " + FloatToStrF(MaxLoadPress, ffFixed,
      6, 2) + "кПа        Наружный диаметр: " + FloatToStrF(OuterD, ffFixed, 6,
      2) + "мм";
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "Статический радиус: " + FloatToStrF(StaticR, ffFixed, 6,
      2) + "мм            Ширина профиля: " + String(ProfileWide) + "мм";
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "   Давление в шине: " + FloatToStrF(InitPressure, ffFixed,
      6, 1) + "кПа                   Обод: " + WheelRim;
   left[LineCnt] = LeftMarg;
   ...
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "        Общее время испытаний: " +
      mSecToHMSStr(TotalTime);
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "        Общий путь: " + FloatToStrF(TotalS, ffFixed, 7,
      0) + "км";
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "        Дата начала испытания: " + Start.FormatString
      ("dd.mm.yyyy hh:nn:ss");
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] = "        Дата окончания испытания: " + Stop.FormatString
      ("dd.mm.yyyy hh:nn:ss");
   left[LineCnt] = LeftMarg;
   wstr[++LineCnt] =
      "+-----+---------+--------+--------+--------+---------+-----------+";
   ...
   for (int i = 0; i < PollsNo; i++)
   {
      ...
      { // это не даст печататься пустым строкам данных (пожелание заказчика)
         wstr[++LineCnt] = "| " + IntToS(i + 1, 3) + " | " + mSecToHMSStr(rT[i])
            + "   | " + FltToStr(rV[i], ffFixed, 6, 2) + " | " + FltToStr(rS[i],
            ffFixed, 6, 0) + " | " + FltToStr(rL[i], ffFixed, 6, 2) + " | " +
            FltToStr(rR[i], ffFixed, 5, 2) + " |    " + FltToStr(rTemp[i],
            ffFixed, 5, 1) + "  |";
         left[LineCnt] = LeftMarg;
      }
   ...
   if (LinePrinted >= 0)
   {
      ...
      for (int PNo = 0; PNo < PNum; PNo++)
      {
         ...
         {
            wstr[++LineCnt] = "| " + IntToS(i + 1, 3) + " | " +
               mSecToHMSStr(rT[i]) + "| " + FltToStr(rV[i], ffFixed, 6, 2) +
               " | " + FltToStr(rS[i], ffFixed, 6, 2) + " | " + FltToStr(rL[i],
               ffFixed, 6, 2) + " | " + FltToStr(rR[i], ffFixed, 5, 0) +
