         ...
         do
         {
            if (!(sr.Name == "." || sr.Name == "..")) // это удалять не надо
                  ...
                  (sr.Attr == faDirectory)) // найдена папка
               {
                  ...
                  // удаляем теперь уже пустую папку
               }
...
String GostToVin(AnsiString Gost)
{
   ...
   Gost = Replace(Gost, " ", "");
   bool isp = false;
   // показывает может ли отсутствовать исполнение в обозначении
   if (Trim(Gost) == "")
   {
      return "";
   }
   else
   {
      ...
      {
         Gost = Gost.SubString(Gost.UpperCase().Pos("ПЕШК"),
            Gost.Length() - Gost.UpperCase().Pos("ПЕШК") + 1);
      ...
      {
         Gost = Gost.SubString(Gost.UpperCase().Pos("ГКНЮ"),
            Gost.Length() - Gost.UpperCase().Pos("ГКНЮ") + 1);
         Gost.Insert("95", Gost.UpperCase().Pos("ГКНЮ"));
         // находим в строке буквенное обозначение и меняем на цифровое
         Gost.Delete(Gost.UpperCase().Pos("ГКНЮ"), 4);
         isp = true;
      }
      if (Gost.UpperCase().Pos("НАЦВ") != 0)
      {
         Gost = Gost.SubString(Gost.UpperCase().Pos("НАЦВ"),
            Gost.Length() - Gost.UpperCase().Pos("НАЦВ") + 1);
      ...
      {
         Gost = Gost.SubString(Gost.UpperCase().Pos("НЕИА"),
            Gost.Length() - Gost.UpperCase().Pos("НЕИА") + 1);
      ...
      if (Gost.Length() <= 15 && Gost.Length() > 5)
      {
         if (isp)
         {
            ...
            {
               Gost = Gost + "0";
            } // если в обозначении отсутствует номер исполнения(нулевое исполнение) то добиваем конец нулями
         }
         else
         {
            while (Gost.Length() < 15)
            {
               Gost = "0" + Gost;
            } // если трока меньше 15 символов добиваем нулями (случай с обозначением стандартных или покупных изделий)
         }
      }
      else
         return tmp;
...
{
   if (Vin.Length() != 15 || GetOnlyNum(Vin).Length() != Vin.Length())
   {
      return Vin;
   }
...
void AutoWidthSG(TStringGrid *sg)
{
   ...
   for (i = 0; i < sg->ColCount; i++)
   {
      ...
      for (j = 0; j < sg->RowCount; j++)
      {
         ...
         {
            sg->ColWidths[i] = 40;
         } // минимальный размерчик под комбо
      }
   }
}
