...
{
  Index=0;                            // текущий индекс таблицы
  for (int i=0,j=7;i<LDQTY;i++) {       // столбец заданных скоростей
    if(i<8) {
      TargetLd[i]=(i)*10;
    }
    else if(i==8) TargetLd[i]=75;
    else {
      TargetLd[i]=(j--)*10;
    }
  }
...
__fastcall LCalibr::LCalibr(void)                  // конструктор
{
  loaded=false;
  Index=0;                            // текущий индекс таблицы
  TargetLd[0]=5;  TargetLd[1]=6; TargetLd[2]=7; TargetLd[3]=8;
  TargetLd[4]=9;
  TargetLd[5]=10;  TargetLd[6]=11; TargetLd[7]=12; TargetLd[8]=13;
  TargetLd[9]=15;  TargetLd[10]=20; TargetLd[11]=30; TargetLd[12]=40;
  TargetLd[13]=50;  TargetLd[14]=70;
  for (int i=15,j=2;i<LDCQTY;i++) {    // столбец заданных нагрузок
    TargetLd[i]=TargetLd[15-(j++)];
  }
