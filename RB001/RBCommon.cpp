...
#endif
extern OPCRW *pOPC=0;// указатель на класс OPCRW
extern CD_9904_interface *cd_9904=0;
extern HANDLE CD_9904Thread=0;
extern bool needSaveA=false,
			needSaveB=false;     //индикаторы автосохранения протоколов за испытание
extern bool StendConnection=false;//индикатор связи со стендом

bool  switch_Carriage1=false;     //триггеры для обработки сообщений остановки каретки
...
String    TyreTypeA="А",
          TyreTypeB="Б";        // тип покрышки
String    RunProgNameA="Прог 1",
          RunProgNameB="Прог 2";// наименвание программы обкатки
...
float  poll_step_S[MAXNUMOFPOLLS] ={0}; // массив опросов по пути
float  prog_step_S[MAXNUMOFSTEPS] ={0}; // массив шагов по пути
float  Ssettings[2][MAXNUMOFSTEPS]={0}; // массив нагрузок (1-й столбец) и скоростей (2-й столбец)
...
int    poll_step_T[MAXNUMOFPOLLS] ={0}; // массив опросов по времени
int    prog_step_T[MAXNUMOFSTEPS] ={0}; // массив шагов по времени
float  Tsettings[2][MAXNUMOFSTEPS]={0}; // массив нагрузок (1-й столбец) и скоростей (2-й столбец)

// переменные группы CtrlGr1 ------------------------------------------------
// MEMORY
bool  bGr1[GR1BOOLITEMSNUM];    // массив булевых значений в памяти и блоке DB10
bool  *Blowout1     =&bGr1[ 0]; //M02.1 Разрыв шины 1
bool  *GuardRail1   =&bGr1[ 1]; //I125.3 Ограждение 1
bool  *Pump1On      =&bGr1[ 2]; //M50.1 Насос 1
bool  *Blowout2     =&bGr1[ 3]; //M03.1 Разрыв шины 2
bool  *GuardRail2   =&bGr1[ 4]; //I125.7 Ограждение 2
bool  *Pump2On      =&bGr1[ 5]; //M50.2 Насос 2
bool  *Carriage1To  =&bGr1[ 6]; //M50.3 Подвод каретки поз 1
bool  *Carriage1From=&bGr1[ 7]; //M50.4 Отвод каретки поз 1
bool  *Carriage2To  =&bGr1[ 8]; //M50.5 Подвод каретки поз 2
bool  *Carriage2From=&bGr1[ 9]; //M50.6 Отвод каретки поз 2
bool  *Carriage1Off =&bGr1[10]; //M02.2 каретка позиция 1 в исходном
bool  *Carriage2Off =&bGr1[11]; //M03.2 каретка позиция 2 в исходном
bool  *ControlLateralA=&bGr1[12];      //M02.3 вкл/выкл контроля бокового разрыва поз А
bool  *ControlLateralB=&bGr1[13];      //M03.3 вкл/выкл контроля бокового разрыва поз Б
bool  *DestroyLateral1=&bGr1[14];      //M34.0 рарушение бововой 1
bool  *DestroyLateral2=&bGr1[15];      //M34.2 разрушение боковой 2
bool  *DestroyRunning1=&bGr1[16];      //M34.1 разрушение беговой 1
bool  *DestroyRunning2=&bGr1[17];      //M34.3 разрушение беговой 2
bool  *OilLvl=&bGr1[18];				//M34.4 уровень масла
bool  *OilFilter=&bGr1[19];             //M34.5 засор фильтра
bool  *ElDriveDrum=&bGr1[20];           //M34.6 нет готовности электропривода

//
float fGr1[GR1FLTITEMSNUM];
float *OilTemp			=&fGr1[0]; //DB20,x100 температура масла
float *ReadLoading_1	=&fGr1[1]; //DB10,REAL26 Нагрузка 1  считывание
float *ReadLoading_2	=&fGr1[2]; //DB10,REAL62 Нагрузка 2  считывание
float *max_speed		=&fGr1[3]; //DB10,REAL138 максимальная скорость в цикле
float *min_speed        =&fGr1[4]; //DB10,REAL__  минимальная скорость в цикле
float *min_load_1		=&fGr1[5]; //DB10,REAL142 мин. нагрузка 1 в цикле
...
bool  *Start1       =&bGr1[23]; //DB10,X2.2 Старт поз 1
bool  *Stop1        =&bGr1[24]; //DB10,X2.3 Стоп поз 1
bool  *Reset1       =&bGr1[25]; //DB10,X2.4 Сброс поз 1
bool  *DrumOn       =&bGr1[26]; //DB10,X2.5 Включить барабан
bool  *DrumOff      =&bGr1[27]; //DB10,X2.6 Выключить барабан
bool  *KeepLoad1    =&bGr1[28]; //DB10,X3.0 Удержание нагрузки 1
bool  *AutoMode2    =&bGr1[29]; //DB10,X38.0 Режим атомат 2
bool  *ManualMode2  =&bGr1[30]; //DB10,X38.1 Режим ручной 2
bool  *Start2       =&bGr1[31]; //DB10,X38.2 Старт поз 2
bool  *Stop2        =&bGr1[32]; //DB10,X38.3 Стоп поз 2
bool  *Reset2       =&bGr1[33]; //DB10,X38.4 Сброс поз 2
...
bool  *ResetProgram1=&bGr1[39]; //M2.7 Сбросить программу на позиции A
bool  *ResetProgram2=&bGr1[40]; //M3.7 Сбросить программу на позиции B
...
int   *fakt_time_1        =&iDB20[ 1]; //DB20,DINT212 факт. время 1
...
int   *fakt_time_2        =&iDB20[ 4]; //DB20,DINT216 факт. время 2
...
int   *next_step_time2    =&iDB20[ 8]; //DB20,DINT152 номер следующего шага опроса по времени 1
int   *next_step_change1  =&iDB20[ 9]; //DB20,DINT180 номер следующего рабочего шага 1
int   *next_step_change2  =&iDB20[10]; //DB20,DINT184 номер следующего рабочего шага 2
...
float *set_speed_1        =&fDB20[ 7]; //DB20,REAL46 заданная скорость 1 на текущем шаге
float *set_loading_1      =&fDB20[ 8]; //DB20,REAL50 заданная нагрузка 1 на текущем шаге
...
float *set_speed_2        =&fDB20[13]; //DB20,REAL86 заданная скорость 2 на текущем шаге
float *set_loading_2      =&fDB20[14]; //DB20,REAL90 заданная нагрузка 2 на текущем шаге
float *next_step_distance1=&fDB20[15]; //DB20,REAL156 номер следующего шага опроса по пробегу 1
float *next_step_distance2=&fDB20[16]; //DB20,REAL160 номер следующего шага опроса по пробегу 2
...
int   *type_cycle_1     =&iDB10[ 2]; //DB10,INT4 Тип цикла 1                  S7:[S7 connection_4]DB10,INT4
int   *StepsQty1        =&iDB10[ 3]; //DB10,INT34 Количество шагов программы 1
int   *PollsQty1        =&iDB10[ 4]; //DB10,INT36 Количество опросов 1
int   *type_cycle_2     =&iDB10[ 5]; //DB10,INT40 Тип цикла 2                 S7:[S7 connection_4]DB10,INT4
...
float *S_end_cycle_1    =&fDB10[ 0]; //DB10,REAL18 Конечный пробег цикла 1    S7:[S7 connection_4]DB10,REAL18
...
float *S_end_cycle_2    =&fDB10[ 4]; //DB10,REAL54 Конечный пробег цикла 2    S7:[S7 connection_4]DB10,REAL18
...
float *CorrAMeasSpeed   =&fDB10[ 9]; //DB10,REAL82 Корекция А по измерению скорости
float *CorrBMeasSpeed   =&fDB10[10]; //DB10,REAL86 Корекция Б по измерению скорости
float *Temp1LowLimit    =&fDB10[11]; //DB10,REAL90 Нижний предел датчика температуры 1
float *Temp1UpLimit     =&fDB10[12]; //DB10,REAL94 Верхний предел датчика температуры 1
float *CorrAMeasLoad1   =&fDB10[13]; //DB10,REAL98 Корекция А по измерению  нагрузки 1
float *CorrBMeasLoad1   =&fDB10[14]; //DB10,REAL102 Корекция Б по измерению нагрузки 1
float *Temp2LowLimit    =&fDB10[15]; //DB10,REAL106 Нижний предел датчика температуры 2
float *Temp2UpLimit     =&fDB10[16]; //DB10,REAL110 Верхний предел датчика температуры 2
float *CorrAMeasLoad2   =&fDB10[17]; //DB10,REAL114 Корекция А по измерению  нагрузки 2
float *CorrBMeasLoad2   =&fDB10[18]; //DB10,REAL118 Корекция Б по измерению нагрузки 2
float *Rad1LowLimit     =&fDB10[19]; //DB10,REAL122 Нижний предел измерений датчика радиуса 1
...
float *A1[GR12ARRAYSIZE];    //DB71,REAL2-172 Коэффициенты коррекции поз. 1    S7:[S7 connection_4]DB10,REAL18
float *Q1[GR12ARRAYSIZE];    //DB71,REAL6-176 границы действия коэффициентов коррекции поз. 1    S7:[S7 connection_4]DB10,REAL18
// адреса переменных в группе 12
wchar_t *Gr12ItemsNme[GR12ITEMSNUM]={//Коэффициент коррекции прямой ход
...
float *A2[GR13ARRAYSIZE];    //DB70,REAL2-172 Коэффициенты коррекции поз. 2    S7:[S7 connection_4]DB10,REAL18
float *Q2[GR13ARRAYSIZE];    //DB70,REAL6-176 границы действия коэффициентов коррекции поз. 2    S7:[S7 connection_4]DB10,REAL18
// адреса переменных в группе 13
wchar_t *Gr13ItemsNme[GR13ITEMSNUM]={//Коэффициент коррекции прямой ход
...
float __fastcall StrToFlt(String ws)    // Преобразование строки в значение типа float
{ String s;
  s=Trim(ws);
  if(s=="") return 0.0;
  int PointPos=s.Pos(".");
