...
// переменные протокола
	...
	String	  ManufactDate;      // дата изготовления (текстом, сразу через :)
	...
	AnsiString DateAsWeekPlusYear (TDateTime date);

	AnsiString CustomDate (void)
	{
		return ManufactDate;
		/* Старая реализация через дату. Теперь эта дата - это строка
		int week_num;
		Word year, month, day;
