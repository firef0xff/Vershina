...
namespace  md5
{
	...
	void  	Get_md5::md5_starts( md5_context *ctx )
    {
		ctx->total[ 0] = 0 ;
        ctx->total[ 1] = 0 ;
	...
	void 	Get_md5::md5_process( md5_context *ctx, uint8 data[64 ] )
    {
        uint32 X[ 16 ], A, B, C, D;
	...
	void  	Get_md5::md5_update( md5_context *ctx, uint8 *input, uint32 length )
    {
		uint32 left, fill;
	...
	void 	Get_md5::md5_finish( md5_context *ctx, uint8 digest[16] )
    {
        uint32 last, padn;
        uint32 high, low;
        uint8 msglen[ 8 ];
